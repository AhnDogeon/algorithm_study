    ...
    for brands in brand_list:
        ...
        if(rescode==200):
            ...
            for _ in range(many_time):
                url = "https://openapi.naver.com/v1/search/local?display=30&query=" + encText + '&start=' + str(start)  # json 결과
                ...
                if(rescode==200):
                    ...
                    for j in range(len(py_rt['items'])):
                        if '서울특별시' in py_rt['items'][j]['address'] and real_gu in py_rt['items'][j]['address'] and code in py_rt['items'][j]['category']:
                            # print(py_rt['items'][j])
